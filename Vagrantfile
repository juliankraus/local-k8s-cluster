...
def string_to_number_in_range(str)
...
Vagrant.configure("2") do |config|
    ...
    vm_configs = [
        { name: "k8s-control-plane", box: "bento/ubuntu-24.04", box_version: "202404.26.0", cmds: ["apt-get update", "apt-get install -y ansible"], ip: "192.168.100.10", mac: "52:54:00:12:34:56" },
        { name: "k8s-worker-node-1", box: "bento/ubuntu-24.04", box_version: "202404.26.0", cmds: ["apt-get update", "apt-get install -y ansible"], ip: "192.168.100.20", mac: "78:8D:0B:3F:30:89"},
        { name: "k8s-worker-node-2", box: "bento/ubuntu-24.04", box_version: "202404.26.0", cmds: ["apt-get update", "apt-get install -y ansible"], ip: "192.168.100.30", mac: "B6:9B:C8:7C:3E:45" }
    ]
    ...
    vm_configs.each do |vm|
        ...
        config.vm.define vm[:name] do |node|
            ...
            node.vm.provision "shell", inline: <<-SHELL
                echo "Running shell commands: #{vm[:cmds].join("\n")}"
                #{vm[:cmds].join("\n")}
            SHELL
            
            # Configure keyboard layout, chrony, user, ssh
            ...
            if vm[:name].include?("control-plane")
               node.vm.provider "qemu" do |qe|
                   ...
                   qe.extra_qemu_args = ["-device", "virtio-net-pci,netdev=net0,mac=" + vm[:mac], "-netdev", "socket,id=net0,fd=3", "-device", "virtio-net-pci,netdev=net1", "-netdev", "user,id=net1,hostfwd=tcp::" + string_to_number_in_range(vm[:name]).to_s + "-:22", "-device", "virtio-net-pci,netdev=net2", "-netdev", "user,id=net2,hostfwd=tcp::6443-:6443"] 
               end
               node.vm.provision "ansible_local" do |ansible|
               ...
               node.vm.provider "qemu" do |qe|
                   ...
                   qe.extra_qemu_args = ["-device", "virtio-net-pci,netdev=net0,mac=" + vm[:mac], "-netdev", "socket,id=net0,fd=3", "-device", "virtio-net-pci,netdev=net1", "-netdev", "user,id=net1,hostfwd=tcp::" + string_to_number_in_range(vm[:name]).to_s + "-:22"] 
               end            
               node.vm.provision "ansible_local" do |ansible|
            ...
            node.vm.provider "qemu" do |qe|
                ...
                qe.cpu = "max"
                qe.net_device = nil
                qe.ssh_port = string_to_number_in_range(vm[:name])                   
            end
        end
    end    
end
